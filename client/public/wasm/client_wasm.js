 ...
 * @returns {string}
 */
export function compute_aggregate_key(pk_hexes) {
    let deferred3_0;
    let deferred3_1;
    try {
 ...
 * @returns {any}
 */
export function encrypt_plaintext(plaintext_hex, pk_hex) {
    const ptr0 = passStringToWasm0(plaintext_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passStringToWasm0(pk_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len1 = WASM_VECTOR_LEN;
...
const WasmClientPlayerFinalization = (typeof FinalizationRegistry === 'undefined')
    ? { register: () => {}, unregister: () => {} }
    : new FinalizationRegistry(ptr => wasm.__wbg_wasmclientplayer_free(ptr, 1));
