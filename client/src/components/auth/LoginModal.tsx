...
import suiLogoSvg from '/sui-sui-logo.svg';
...
const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose }) => {
  ...
  return ReactDOM.createPortal(
    <ModalWrapper
      ...
      onClick={(e) => {
        if ((e.target as HTMLElement).id === 'login-modal-wrapper') {
          onClose();
        }
      }}
    >
      <StyledModal>
        ...
        <ModalContent>
          ...
          <WalletSection>
            ...
            <WalletTitle>{t('login_wallet-title')}</WalletTitle>
            <WalletDesc>{t('login_wallet-desc')}</WalletDesc>
            <ConnectButton />
          </WalletSection>
          ...
          {availableProviders.length > 0 && (
            <>
              ...
              <OAuthSection>
                {availableProviders.map((p) => (
                  <OAuthButton
                    key={p.key}
                    $provider={p.key}
                    onClick={() => loginWithZkLogin(p.key)}
                  >
                    {p.icon}
                    {p.label}
                  </OAuthButton>
              ...
              <OAuthNote>
                {t('login_oauth-note-1')}
                {' '}
                {t('login_oauth-note-2')}
              </OAuthNote>
            </>
          )}
        </ModalContent>
