...
use crate::pokergame::game_state::{ElGamalCiphertextJson, ReconstructPhase, MaskAndShuffleRoundJson, ShuffleProofJson,PlayerReadableCardJson,
    PkProofJson, ReconstructProofJson, RevealPhase, ShufflePublicState};
...
#[derive(Debug, Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
struct RedealRequestPayload {
    table_id: u32,
    player_pk: String,
    failed_card_indices: Vec<usize>,
}


#[derive(Debug, Clone, Deserialize)]
struct ReconstructSubmitPayload {
    table_id: u32,
    pk_hex: String,
    output_cards: Vec<ElGamalCiphertextJson>,
...
#[derive(Debug, Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
struct ReconstructInitiatePayload {
    table_id: u32,
...
#[derive(Debug, Clone, Serialize)]
struct ReconstructNoticePayload {
    table_id: u32,
    completed_players: Vec<String>,
    pending_players: Vec<String>,
    cards: Vec<String>,
    coefficient_hex: String,
    player_readable_cards: HashMap<String, PlayerReadableCardJson>,
}

#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
struct ReconstructResultPayload {
    table_id: u32,
    completed_players: Vec<String>,
    reconstructed: bool,
}
...
impl GameState {
    /// 根据 pk_hex 移除玩家：从 players map 和 table 中同时移除，返回被移除玩家的 socket_id
    fn remove_player_by_pk(&mut self, pk_hex: &str) -> Option<String> {
        let socket_id = self.players.values()
...
impl SocketState {
    ...
    pub async fn start_game_loop_from_ctx(&self, state: Arc<SocketState>, table_id: u32) {
        let io = match SOCKET_IO.get().cloned() {
            Some(io) => io,
            None => {
                tracing::warn!("[start_game_loop_from_ctx] SocketIo not initialized");
                return;
            }
        };
    ...
    pub async fn find_socket_id_by_pk(&self, pk_hex: &str) -> Option<String> {
        let gs = self.state.read().await;
        gs.players.values()
            .find(|p| p.pk_hex == pk_hex)
            .map(|p| p.socket_id.clone())
    ...
    pub async fn is_player_in_seat(&self, pk_hex: &str) -> bool {
        let gs = self.state.read().await;
    ...
    pub async fn find_player_by_pk(&self, pk_hex: &str) -> Option<Player> {
        let gs = self.state.read().await;
        gs.players.values()
            .find(|p| p.pk_hex == pk_hex)
            .cloned()
    ...
    pub async fn add_player_to_table(&self, table_id: u32, player: Player) -> Result<usize, String> {
    ...
    pub async fn mark_reveal_complete_for_pk(&self, table_id: u32, pk_hex: &str) -> Result<bool, String> {
        let mut gs = self.state.write().await;
        if let Some(table) = gs.tables.get_mut(&table_id) {
    ...
    pub async fn broadcast_hand_reveal_result(&self, table_id: u32) {
        ...
        for (player_pk, cards) in player_cards {
            tracing::debug!("Player {} revealed cards: {:?}", player_pk, socket_id_map);
            ...
            if let Ok(sid) = socket_id.parse::<socketioxide::socket::Sid>() {
                if let Some(socket) = io.get_socket(sid) {
                    tracing::debug!("[broadcast_hand_reveal_result] socket  found for player {}, socket_id={}", player_pk, socket_id);
                    let _ = socket.emit(actions::HAND_REVEAL_RESULT, &payload);
                }else{
                    tracing::debug!("[broadcast_hand_reveal_result] socket not found for player {}, socket_id={}", player_pk, socket_id);
                }
            }else{
                tracing::debug!("[broadcast_hand_reveal_result] socket_id {} is not a valid sid", socket_id);
            }
        }
    }
    ...
    pub async fn broadcast_redeal_notice(&self, table_id: u32) {
        let io = match SOCKET_IO.get() {
            Some(io) => io.clone(),
            None => {
                tracing::warn!("[broadcast_redeal_notice] SocketIo not initialized");
                return;
            }
        };
    ...
    pub async fn broadcast_redeal_result(&self, table_id: u32) {
        let io = match SOCKET_IO.get() {
            Some(io) => io.clone(),
            None => {
                tracing::warn!("[broadcast_redeal_result] SocketIo not initialized");
                return;
            }
        };
        ...
        for (player_pk, cards) in player_cards {
            ...
            if let Ok(sid) = socket_id.parse::<socketioxide::socket::Sid>() {
                if let Some(socket) = io.get_socket(sid) {
                    let _ = socket.emit(actions::REDEAL_RESULT, &payload);
                }
            }
        }
    }
...
async fn broadcast_to_table(io: &SocketIo, state: &Arc<SocketState>, table_id: u32, message: Option<&str>) {
    let table_views = {
        ...
        table.players.iter()
            .map(|p| (p.socket_id.clone(), hide_opponent_cards(table, &p.socket_id)))
            .collect::<Vec<_>>()
    };
...
async fn handle_interrupts(io: &SocketIo, state: &Arc<SocketState>, table_id: u32, reconstruct_active: bool, shuffle_active: bool, reveal_active: bool) -> Option<bool> {
    if reconstruct_active {
        ...
        if let Some(completed) = reconstruct_result {
            if completed {
                ...
                // Start shuffle phase after reconstruct completes (same as RECONSTRUCT_SUBMIT handler)
                let shuffle_notice_data = {
                    ...
                    if let Some(table) = gs.tables.get_mut(&table_id) {
                        ...
                        let shuffle_socket_id = current_pk.as_ref().and_then(|pk| {
                            gs.players.values().find(|p| p.pk_hex == *pk).map(|p| p.socket_id.clone())
                        });
                        shuffle_state.zip(shuffle_socket_id)
                    } else {
                        None
                    }
                };
    ...
    if shuffle_active {
        ...
        if let Some(timed_out_pk) = timeout_result {
            ...
            let should_stop_early = {
                let mut gs = state.state.write().await;
                gs.remove_player_by_pk(&timed_out_pk);
    ...
    if reveal_active {
        ...
        if let Some(timed_out_pks) = timeout_result {
            ...
            if is_redeal_reveal {
                ...
                broadcast_to_table(io, state, table_id, Some("Redeal reveal timed out")).await;
                return Some(true);
            }
            ...
            let should_reset_hand = {
                let mut gs = state.state.write().await;
                for timed_out_pk in &timed_out_pks {
                    if let Some(socket_id) = gs.remove_player_by_pk(timed_out_pk) {
                        if let Ok(sid) = socket_id.parse::<socketioxide::socket::Sid>() {
                            if let Some(socket) = io.get_socket(sid) {
            ...
            if active_count >= MIN_START_NUM as usize {
                ...
                if let Some(ready_at) = ready_at {
                    ...
                    if let (Some(st), Some(pk)) = (shuffle_state, current_pk) {
                        let socket_id = {
                            let gs = state_c.state.read().await;
                            gs.players.values().find(|p| p.pk_hex == pk).map(|p| p.socket_id.clone())
                        };
...
fn on_connect(socket: SocketRef, _io: SocketIo, _state: Arc<SocketState>) {
        ...
        let (should_start, chips_deduct, sit_msg) = {
            ...
            if let Some(table) = gs.tables.get_mut(&payload.table_id) {
                if let Some((player_clone, player_name, player_id)) = player_data {
                    ...
                    (table.active_players().len() == MIN_START_NUM as usize, Some((player_id, payload.amount)), Some(msg))
    ...
    socket.on(actions::SHUFFLE_SUBMIT, async move |s: SocketRef, Data::<serde_json::Value>(data), io: SocketIo, State(state): State<Arc<SocketState>>| {
        ...
        match payload {
            Ok(payload) => {
                ...
                let result = state.submit_verified_shuffle_for_pk(payload.table_id, &payload.pk_hex,player, payload.output_cards.clone(), payload.shuffle_proof.clone()).await;
    ...
    socket.on(actions::RECONSTRUCT_SUBMIT, async move |s: SocketRef, Data::<ReconstructSubmitPayload>(payload), io: SocketIo, State(state): State<Arc<SocketState>>| {
        let socket_id = s.id.to_string();
        tracing::debug!("[RECONSTRUCT_SUBMIT] request received, pk_hex={}, table_id={}", payload.pk_hex, payload.table_id);
        let pk_hex = {
            let gs = state.state.read().await;
            gs.players.get(&socket_id).map(|p| p.pk_hex.clone())
        };
        let pk_hex = match pk_hex {
            Some(pk) => pk,
            None => {
                tracing::warn!("[RECONSTRUCT_SUBMIT] Player {} not found", socket_id);
                return;
            }
        };

        let (all_complete, reconstruct_payload, shuffle_notice_data) = {
            let mut gs = state.state.write().await;
            if let Some(table) = gs.tables.get_mut(&payload.table_id) {
                let is_complete = table.submit_reconstruct_deck(pk_hex.as_str(), payload.output_cards.clone(), payload.swap_cards.clone(), payload.proof).map_err(|e| tracing::error!("[RECONSTRUCT_SUBMIT] Error: {}", e)).unwrap_or(false);
                if is_complete {
                    let reconstruct_payload = ReconstructResultPayload {
                        table_id: payload.table_id,
                    ...
                    let shuffle_notice = shuffle_state.zip(shuffle_socket_id);
                    (is_complete, Some(reconstruct_payload), shuffle_notice)
                } else {
        ...
        if let Some((shuffle_state, socket_id)) = shuffle_notice_data {
            if let Ok(sid) = socket_id.parse::<socketioxide::socket::Sid>() {
                if let Some(socket) = io.get_socket(sid) {
                    let notice = ShuffleNoticePayload { table_id: payload.table_id, shuffle_state: Some(shuffle_state) };
                    let _ = socket.emit(actions::SHUFFLE_NOTICE, &notice);
                }
            }
        }
    ...
    socket.on(actions::REDEAL_REQUEST, async move |_s: SocketRef, Data::<RedealRequestPayload>(payload), io: SocketIo, State(state): State<Arc<SocketState>>| {
        tracing::info!("[REDEAL_REQUEST] Player {} requests redeal for {} failed cards on table {}",
            payload.player_pk, payload.failed_card_indices.len(), payload.table_id);

        // 执行 redeal
        let redealt_indices = {
            let mut gs = state.state.write().await;
            if let Some(table) = gs.tables.get_mut(&payload.table_id) {
                match table.redeal_cards_for_player(&payload.player_pk, payload.failed_card_indices.clone()) {
                    Ok(indices) => indices,
                    Err(e) => {
        ...
        if !redealt_indices.is_empty() {
            // 启动 redeal reveal 阶段
            {
                let mut gs = state.state.write().await;
                if let Some(table) = gs.tables.get_mut(&payload.table_id) {
                    table.start_redeal_reveal_phase(&payload.player_pk, redealt_indices);
                }
            }

            // 广播 redeal notice 给所有玩家
            state.broadcast_redeal_notice(payload.table_id).await;
            broadcast_to_table(&io, &state, payload.table_id, Some("Redeal requested, new cards being dealt")).await;
        }
    });
    ...
    socket.on(actions::RECONSTRUCT_INITIATE, async move |_s: SocketRef, Data::<ReconstructInitiatePayload>(payload), io: SocketIo, State(state): State<Arc<SocketState>>| {
        ...
        match result {
            Ok(()) => {
                ...
                broadcast_to_table(&io, &state, payload.table_id, Some("Reconstruct vote initiated")).await;
            }
            Err(e) => {
    ...
    socket.on_disconnect(async move |s: SocketRef, io: SocketIo, State(state): State<Arc<SocketState>>| {
        let socket_id = s.id.to_string();
        let (auto_fold_table_ids, _user_id, affected_table_ids, _need_cleanup): (Vec<u32>, Option<String>, Vec<u32>, bool) = {
            let mut gs = state.state.write().await;
